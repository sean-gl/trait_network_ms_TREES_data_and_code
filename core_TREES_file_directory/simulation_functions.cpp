...
//this is where roots are disconnected
	...
	do //coupled plant water balance and carbon assimilation
	{
		...
		else
		{
			...
			for (mm = 1; mm <= totmodules+1; mm++)
			{
				if ( mm != ShootModules+1)  // skip over "hidden" module   [sean] hidden?
				{
					ks2[mm][1] = ks_in[mm][1];
					ks2[mm][0] = ks_in[mm][0];
...
//water uptake in each rhizosphere contributing to transpiration
		...
		//[sean] adding this bit to remove transpiration lost to weeds
		if(transpiration > 0) {
			thetaSoil[i] = thetaSoil[i] - ((transpiration)/(rootDepth[i]*mineral_fraction))*0;//[sean] assuming 20% of T is weed T
		}
		//cout << "transpiration = " << transpiration << "  delta_Sr = " << delta_Sr << endl; //[sean]
		//[sean] transpiration can be + or - (why?).  When positive it removes water from soil.
		//[sean] Thus, we only want to remove weed T when transpiration is positive.
	}
...
//
	...
	//[sean] changing this to 1 here to avoid RootAllocation and CN becoming NaN in bgc.cpp
	N_avail_rate_plant = 1; //[sean]
