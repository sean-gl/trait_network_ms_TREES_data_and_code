...
//---------------------------------------------------------------------------------------------------
void simulator(Data_Store& in_data, //[sean] current_params is a pointer
		 State_Store& state,
		 Parameter* current_params,
...
//Initializing state variables for HydraulicModel
        for(j=0; j < n_tsteps; j++)
        {
                EcState.push_back(0.0);  //[sean]...not sure about the syntax here. EcState is a vector of doubles
        }
        //set to 50 because model should take nowhere near this long to converge.
        for(j=0; j < 50; j++)
        {
...
for (j = 0; j < treesParams.rmodules; j++)
{
	if(j==0) { thetaSoil[j] = 0.20; }
	if(j==1) { thetaSoil[j] = 0.20; }
	if(j==2) { thetaSoil[j] = 0.20; }
	if(j==3) { thetaSoil[j] = 0.20; }
	if(j==4) { thetaSoil[j] = 0.20; }
}
