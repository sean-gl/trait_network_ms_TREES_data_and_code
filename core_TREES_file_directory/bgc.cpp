...
double BiogeochemicalCycles::C4photosynthesis(trees_params treesParams,
										...
										void BiogeochemicalCycles::computeMaximumRootBiomassCarbon(trees_params treesParams)
										{
											...
											for (int j = 0; j < nRoots; ++j)
											{
												...
												for (int k = 0; k < nRootOrders; ++k)
												{
													...
													//if(j == nRoots-1) {	allocationScalar /= 6.0; } //6 for no rooots, turn off for deep roots
	...
	void BiogeochemicalCycles::computeLeafAllocation(trees_params& treesParams,
		...
		double SLA)
		{
			...
			{
				N_avail_rate_plant *= 2.0; //[sean] default *= 2.0;
			}
			else
			{
...
//			if (yday > 163 && yday < 215) //[sean] default= >163, <215
			if (yday > 163) //[sean] replacing above line with this one (prevent N status < 1)
			{
				for (int k = 0; k < nRootOrders; k++)
				{
					// rhizosphereNitrateNitrogen[0][k] += 0.05/48.0; //[sean] commenting this out and replacing with if-else below
					//[sean] adding this if-else to prevent rhizosphere N from getting too high or too low (both cases are bad)
					if(getPlantN() < 50) {  //[sean] default=50
						rhizosphereNitrateNitrogen[0][k] += 0.7/48.0; //[sean]
						rhizosphereNitrateNitrogen[1][k] += 2/48.0; //[sean] change N (numerator) here for unlimited N. default = 0.05/48.0
					}
					else {
						rhizosphereNitrateNitrogen[0][k] -= 0.05/48.0; //[sean]
						rhizosphereNitrateNitrogen[1][k] -= 0.05/48.0; //[sean] change N (numerator) here for unlimited N. default = 0.05/48.0
					}
				}
			}
...
//					if (yday > 210 || (yday > 203 && treesParams.live_lai > treesParams.lai_at_full_canopy_height))
					...
					if (yday > 210) //[sean] turn off above line and replace with this: 210 or 196 (14 days eralier GDU from 2600 to 2500)
					...
					//[sean] adding this reduce allocation more sharply as the end of the season approaches
				...
				//--------------------------------------
				leafCfraction = leafCfraction*N_avail_rate_plant*kratio; //[sean] kratio is the relative (to max) hydraulic conductance
		...
		double stemAllocation)
		{
			...
			for (int j = 0; j < nRoots; j++)
			{
				...
				for (int k = 0; k < nFineRootOrders; k++)
				{
					...
					// cout << "N_avail_rate_plant =" << N_avail_rate_plant << endl; //[sean]
					...
					//if(j == nRoots-2) { rootAllocation = 0.001; }  // for no roots use 0.001.  For deep and dense use 0.1
					//if(j == nRoots-3) { rootAllocation = 0.001; }  // for no roots use 0.001.  For deep and dense use 0.1
