...
double calc_gvc(double Kl,
	...
	double pair)
	{
		...
		if (soilpsi < 0.0 && Kl > 0.0 && leafpsi < soilpsi && !isnan(leafpsi))
		{
			...
			//[sean] of water increases..., which is probably not the response we
			//[sean] should expect to protect the xylem from damage... Add a wind back fxn?
...
//			      /*
			...
			// also creating variables to save logistic rate and midpoint.  (2.5, 3.5) (1.5, 2.0)
			float sean_gs_turn = -2.5; // point of **intial** stomatal closure (MPa) [-2.0]
			...
			float sean_midpoint = -3.5;  // logistic midpoint (MPa) [-3.0]
			...
			if(leafpsi < sean_gs_turn) {
				gs = sean_gs_max / (1 + exp(sean_rate * (leafpsi - sean_midpoint)));
			}

			//[sean] adding a min gs that is greater than zero (cuticle g)
			if(gs <= 0.01 * 303) { gs = 0.01 * 303; }
//			      */  //[sean] and ends here
...
double capillary_rise(double water_table_depth,
	...
	double sat_hydraulic_cond)
	{
		...
		water_table_depth  = water_table_depth  * 100.0; //[sean] could change this.  Default=100
	...
	double ks)
	{
		...
		while (kiter > 0)
		{
			...
			for (int i = rmodules-1; i > 0; i--)
			{
				...
				//sleep(1);
				//delta_Sr = 0; //[sean] turn **on** to stop the the upward movement of water
